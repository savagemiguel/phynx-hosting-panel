...
create_ssl_certificate() {
    ...
    if certbot certonly --apache --non-interactive --agree-tos --email "admin@$DOMAIN" -d "$DOMAIN" -d "www.$DOMAIN" -d "$PANEL_SUBDOMAIN" -d "$PHYNXADMIN_SUBDOMAIN" 2>/dev/null; then
        ...
        openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
            ...
            -subj "/C=US/ST=State/L=City/O=Organization/OU=OrgUnit/CN=$DOMAIN/emailAddress=$"
