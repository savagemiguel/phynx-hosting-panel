...
secure_mysql_installation() {
    ...
    PHYNX_DB_PASS=$(openssl rand -base64 24 | tr -d "=+/" | cut -c1-20)
    ...
    if [[ "$INSTALL_PHYNX" == "yes" ]]; then
        mysql -u root -p"$MYSQL_ROOT_PASS" -e "CREATE USER IF NOT EXISTS '$PHYNX_DB_USER'@'localhost' IDENTIFIED BY '$PHYNX_DB_PASS';"
        mysql -u root -p"$MYSQL_ROOT_PASS" -e "GRANT ALL PRIVILEGES ON *.* TO '$PHYNX_DB_USER'@'localhost' WITH GRANT OPTION;"
    fi
...
DB_USER="$DB_USER"
DB_PASSWORD="$DB_PASS"
PHYNX_DB_USER="$PHYNX_DB_USER"
PHYNX_DB_PASSWORD="$PHYNX_DB_PASS"
EOF
...
deploy_custom_PHYNX() {
    ...
    if [[ -d "PHYNX" ]]; then
        ...
        cp -r PHYNX "$PHYNX_DIR"
        
        # Set proper ownership and permissions
        chown -R www-data:www-data "$PHYNX_DIR"
        find "$PHYNX_DIR" -type d -exec chmod 755 {} \\;
        find "$PHYNX_DIR" -type f -exec chmod 644 {} \\;
        
        # Create necessary directories for PHYNX
        mkdir -p "$PHYNX_DIR"/{tmp,uploads,save,upload}
        chown -R www-data:www-data "$PHYNX_DIR"/{tmp,uploads,save,upload}
        chmod 777 "$PHYNX_DIR"/{tmp,uploads,save,upload}
        
        # Configure your custom PHYNX if config template exists
        if [[ -f "$PHYNX_DIR/config.sample.php" ]]; then
            source /root/.phynx_credentials
            
            # Create PHYNX config from template with database credentials
            sed "s/{{PHYNX_DB_USER}}/$PHYNX_DB_USER/g; s/{{PHYNX_DB_PASSWORD}}/$PHYNX_DB_PASS/g" \\
                "$PHYNX_DIR/config.sample.php" > "$PHYNX_DIR/config.inc.php"
            
            chown www-data:www-data "$PHYNX_DIR/config.inc.php"
            chmod 644 "$PHYNX_DIR/config.inc.php"
        fi
        
        ok "Custom PHYNX deployed at $PHYNX_DIR"
    else
...
server {
    ...
    location /PHYNX {
        alias $PHYNX_DIR;
        try_files \$uri \$uri/ /PHYNX/index.php?\$query_string;
    }
    
    location ~ /PHYNX/.*\\.php\$ {
        fastcgi_pass unix:/run/php/php8.1-fpm.sock;
        fastcgi_index index.php;
        fastcgi_param SCRIPT_FILENAME $PHYNX_DIR\$fastcgi_script_name;
...
display_installation_summary() {
    ...
    if [[ "$INSTALL_PHYNX" == "yes" ]]; then
        echo -e "phpMyAdmin URL: ${GREEN}http://$PANEL_DOMAIN/PHYNX${NC}"
    fi
    ...
    if [[ "$INSTALL_PHYNX" == "yes" ]]; then
        echo -e "phpMyAdmin User: ${BLUE}$PHYNX_DB_USER${NC}"
        echo -e "phpMyAdmin Password: ${YELLOW}$PHYNX_DB_PASS${NC}"
    fi
...
phpMyAdmin: http://$PANEL_DOMAIN/PHYNX (if enabled)
...
- phpMyAdmin User: $PHYNX_DB_USER
- phpMyAdmin Password: $PHYNX_DB_PASS
...
show_help() {
    ...
    echo "  --no-PHYNX                    Skip custom PHYNX deployment"
    ...
    echo "  $0 --no-PHYNX --csf                   # Skip phpMyAdmin, use CSF firewall"
}
...
main() {
    ...
    echo "• Deploy custom PHYNX: $INSTALL_PHYNX"
